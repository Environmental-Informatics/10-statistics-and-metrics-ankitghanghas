...
def GetAnnualStatistics(DataDF):
    ...
    col_name=['site_no', 'Mean Flow', 'Peak Flow', 'Median', 'Coeff Var', 'Skew', 'TQmean', 'R-B Index', '7Q', '3xMedian']
    ...
    WYDataDF['Median'] = df['Discharge'].median()
...
def GetMonthlyStatistics(DataDF):
    ...
    MoDataDF = pd.DataFrame(0, index = monthlydata.index, columns=col_name)
    df= DataDF.resample('M')
